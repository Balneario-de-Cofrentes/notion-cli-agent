...
- **`bulk delete` archived the wrong database.** It re-invoked the `archive` subcommand with the command name as the first argument (`parseAsync(['archive', databaseId, …], { from: 'user' })`), so `archive` received the literal string `"archive"` as its `<database_id>` and resolved *that* against the workspace registry. Commander 12 silently ignored the extra argument, which is why it went unnoticed; commander 15 rejects it. Two tests covered `bulk delete` without asserting which database was queried.
