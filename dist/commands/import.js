...
function frontMatterToProperties(frontMatter, schema, titlePropName) {
    const properties = {};
    for (const [key, value] of Object.entries(frontMatter)) {
        // Skip notion-specific metadata
        if (key === 'notion_id' || key === 'notion_url')
            continue;
...
export function registerImportCommand(program) {
    ...
    importCmd
        ...
        try {
            ...
            // Get database schema
            const db = await client.get(`databases/${options.to}`);
            ...
            for (const file of files) {
                try {
                    ...
                    failed++;
                    console.error(`\n❌ Failed to import ${path.basename(file)}: ${error.message}`);
                }
            }
    ...
    importCmd
        ...
        try {
            ...
            if (options.replace) {
                ...
                const existing = await client.get(`blocks/${options.to}/children`);
