...
function cleanBlockForDuplication(block) {
...
async function duplicateBlocksRecursive(client, sourceBlocks, targetParentId) {
    let count = 0;
...
export function registerDuplicateCommand(program) {
    ...
    duplicate
        ...
        try {
            ...
            const sourcePage = await client.get(`pages/${pageId}`);
            ...
            if (blockCount > 0)
                console.log(`   Copied ${blockCount} blocks`);
        }
        catch (error) {
    ...
    duplicate
        ...
        try {
            ...
            const sourceDb = await client.get(`databases/${databaseId}`);
            ...
            for (const [name, schema] of Object.entries(sourceDb.properties)) {
                ...
                switch (schema.type) {
                    ...
                    case 'rich_text':
                        ...
                        break;
                    case 'number':
                        propConfig.number = { format: schema.number?.format || 'number' };
                        break;
                    case 'select':
                        propConfig.select = { options: schema.select?.options || [] };
                        break;
                    case 'multi_select':
                        propConfig.multi_select = { options: schema.multi_select?.options || [] };
                        break;
                    case 'status':
                        propConfig.status = {
                            options: schema.status?.options || [],
                            groups: schema.status?.groups || [],
                        };
                        break;
                    case 'date':
                        propConfig.date = {};
        ...
        try {
            ...
            for (const [name, schema] of Object.entries(sourceDb.properties)) {
                ...
                switch (schema.type) {
                    ...
                    case 'multi_select':
                        ...
                        break;
                    case 'status':
                        propConfig.status = { options: schema.status?.options || [], groups: schema.status?.groups || [] };
                        break;
                    case 'date':
                        propConfig.date = {};
                        break;
            ...
            for (const entry of entries) {
                try {
                    ...
                    const newPage = await client.post('pages', pageData);
                    // Copy content if requested
                    if (options.content) {
