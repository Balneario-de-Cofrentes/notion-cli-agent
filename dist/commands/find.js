...
function findProperty(properties, types, nameHints) {
...
function findStatusValue(properties, statusPropName, targetStatus) {
...
export function registerFindCommand(program) {
    program
        ...
        try {
            ...
            const db = await client.get(`databases/${options.database}`);
            ...
            if (parsed.statusFilter) {
                const statusProp = findProperty(schema, ['status', 'select'], ['status', 'estado', 'state']);
                if (statusProp) {
            ...
            if (parsed.assigneeFilter === 'empty') {
                const assigneeProp = findProperty(schema, ['people'], ['assignee', 'asignado', 'owner', 'responsible', 'assigned']);
                if (assigneeProp) {
                    filters.push({
            ...
            if (parsed.dateFilter) {
                const dateProp = findProperty(schema, ['date'], ['deadline', 'due', 'fecha', 'date', 'vencimiento', 'due date']);
            ...
            if (parsed.priorityFilter) {
                const priorityProp = findProperty(schema, ['select'], ['priority', 'prioridad', 'importance', 'importancia']);
                if (priorityProp) {
                    // Find matching priority value
                    const propData = schema[priorityProp].select;
                    const options = propData?.options || [];
                    const match = options.find(o => o.name.toLowerCase().includes(parsed.priorityFilter.toLowerCase()));
                    if (match) {
                        filters.push({
            ...
            if (filter)
                body.filter = filter;
            const result = await client.post(`databases/${options.database}/query`, body);
            // Output
            if (options.json) {
