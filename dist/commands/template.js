...
function cleanBlockForTemplate(block) {
...
async function fetchBlocksRecursive(client, blockId) {
...
function cleanBlocksForTemplate(blocks) {
    return blocks.map(block => {
        const clean = cleanBlockForTemplate(block);
        if (block.children) {
            clean.children = cleanBlocksForTemplate(block.children);
        }
        return clean;
    });
}
export function registerTemplateCommand(program) {
    ...
    template
        ...
        try {
            ...
            const templateData = {
                ...
                propertyTypes: Object.fromEntries(Object.entries(page.properties).map(([name, prop]) => [
                    name,
                    prop.type,
                ])),
            };
    ...
    template
        ...
        try {
            ...
            const page = await client.post('pages', pageData);
    ...
    template
        ...
        console.log('\nBlock structure:');
        const showBlocks = (blocks, indent = 0) => {
            const pad = '  '.repeat(indent);
            for (const block of blocks.slice(0, 10)) {
                console.log(`${pad}- ${block.type}`);
                if (block.children && Array.isArray(block.children)) {
                    showBlocks(block.children, indent + 1);
                }
            }
            if (blocks.length > 10) {
