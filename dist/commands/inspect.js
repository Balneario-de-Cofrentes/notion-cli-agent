...
function formatPropertyType(prop) {
    ...
    switch (type) {
        ...
        case 'relation': {
            const relatedDb = data?.database_id || 'unknown';
            return `relation → ${relatedDb.slice(0, 8)}...`;
        }
        case 'rollup': {
            const rollupProp = data?.rollup_property_name || '';
            const relationProp = data?.relation_property_name || '';
