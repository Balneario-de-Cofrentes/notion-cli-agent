...
export function registerAICommand(program) {
    ...
    ai
        ...
        try {
            ...
            for (const [name, value] of Object.entries(page.properties)) {
                const val = getPropertyValue(value);
                if (val && !['title'].includes(value.type)) {
                    keyProps[name] = val;
                }
            }
    ...
    ai
        ...
        try {
            const client = getClient();
            const fields = options.schema.split(',').map((f) => f.trim().toLowerCase());
            // Get page
            const page = await client.get(`pages/${pageId}`);
            const extracted = {};
        ...
        try {
            ...
            for (const [name, schema] of Object.entries(db.properties)) {
                ...
                if (schema.type === 'select') {
                    ...
                    const opts = schema.multi_select?.options || [];
                    info.options = opts.map(o => o.name);
                }
    ...
    ai
        ...
        .action(async (databaseId, description, _options) => {
        try {
            const client = getClient();
            const db = await client.get(`databases/${databaseId}`);
            ...
            for (const [name, schema] of Object.entries(db.properties)) {
                if (schema.type === 'status')
                    statusProp = name;
                if (schema.type === 'date' && !dateProp)
                    dateProp = name;
                if (schema.type === 'people' && !peopleProp)
                    peopleProp = name;
            }
