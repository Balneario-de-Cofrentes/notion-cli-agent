    ...
    program
        ...
        try {
            ...
            do {
                ...
                const result = await client.post(`databases/${databaseId}/query`, queryBody);
            ...
            for (const entry of entries) {
                ...
                if (options.content) {
                    try {
                        ...
                        pageData.content_error = error.message;
                    }
                }
                // Save based on format
                if (options.format === 'markdown') {
                    const mdContent = generateMarkdown(entry, pageData.content);
...
function generateMarkdown(page, blocks) {
...
function extractPropertyValue(prop) {
    ...
    switch (type) {
        case 'title':
        case 'rich_text':
            return data?.map(t => t.plain_text).join('') || null;
        case 'select':
        case 'status':
