...
function parseWhereClause(where, schema) {
    ...
    for (const condition of conditions) {
        ...
        // Build the filter
        const filter = { property: trimmedName };
...
export function registerBulkCommand(program) {
    ...
    bulk
        ...
        try {
            ...
            // Parse set clause
            const setProperties = parseProperties(options.set.split(',').map((s) => s.trim()));
            ...
            for (const page of result.results) {
                try {
                    ...
                    failed++;
                    console.error(`\n❌ Failed to update ${page.id}: ${error.message}`);
                }
            }
        ...
        try {
            ...
            for (const page of result.results) {
                try {
                    ...
                    failed++;
                    console.error(`\n❌ Failed to archive ${page.id}: ${error.message}`);
                }
            }
