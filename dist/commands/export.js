...
import { richTextToMarkdown, } from '../utils/markdown.js';
...
function propertyToValue(prop) {
    ...
    switch (type) {
        case 'title':
        case 'rich_text':
            return richTextToMarkdown(data);
        case 'number':
            return data;
        case 'select':
        ...
        case 'files':
            return data?.map(f => f.file?.url || f.external?.url || f.name) || [];
        case 'relation':
...
function generateFrontmatter(page, includeId = true) {
    const lines = ['---'];
    if (includeId) {
    ...
    for (const [name, value] of Object.entries(page.properties)) {
        const prop = value;
        if (prop.type === 'title')
            continue; // Title is the filename
